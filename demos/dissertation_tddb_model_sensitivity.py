...
DATA_FILES = {
    1.05: 'tddb_1.05_report',
    1.075: 'tddb_1.075_report',
    1.1: 'tddb_1.1_report',
    1.125: 'tddb_1.125_report',
    1.15: 'tddb_1.15_report',
}
...
def defect_generator_demo_model(time, temp, v_g, t_diel, c, bond_strength, thermal_dist):
    ...
    return np.where(Uniform().sample(prob.size).reshape(prob.shape) < prob, 1, 0)
...
def oxide_failed(
    ...
    layout = np.stack([np.stack([defect0, defect1, defect2]), np.stack([defect3, defect4, defect5]),
                      np.stack([defect6, defect7, defect8]), np.stack([defect9, defect10, defect11])], axis=0)
    # Determine defects formed based on whether the stochastic defect formation value has hit a threshold
    oxide = np.where(layout > threshold, 1, 0)
    # Identify if any of the columns in our oxide layer have fully defected
    failed = np.any(np.all(oxide == 1, axis=1), axis=0)
    # The transistor gate oxide breaks down if a conductive channel has formed in a column
    return np.where(failed | np.equal(init, 0), 0, 1)
...
def single_test(step_val, test):
    ...
    field_env = PhysTestEnv(
        ...
        meas_instms={'temp': MeasInstrument(error=Normal(0, 1), precision=4, meas_lims=(-40, 150))},
    )
    ...
    )  # <-- This is what we are schmooing
    defect_dict = {'defect' + str(i): defect_mdl for i in range(12)}
...
def run_simulation(save_files: dict = None):
    ...
    check_for_fails = MeasSpec({'tddb': NUM_SAMPLES}, {'temp': 35 + CELSIUS_TO_KELVIN, 'v_g': 0.9}, name='FailCheck')

    field_use_sim = TestSpec(
        [check_for_fails],
        name='Field Use Sim',
        description='Test spec designed to represent a \
    ...
    with Pool(processes=5) as pool:
        rprts = list(pool.starmap(single_test, [(step, field_use_sim) for step in DATA_FILES]))
...
def visualize(rprts):
    ...
    for_lbci = fail_event_data.replace(to_replace='right-censored', value=174721)
    ...
    for i, prm_val in enumerate(clr_map):
        ...
        fits[prm_val] = rel_lib.Fitters.Fit_Weibull_2P(
            observed['time'].tolist(),
            right_censored=unobserved['time'].tolist(),
            quantiles=True,
            print_results=False,
            show_probability_plot=True,
            color=clr_map[prm_val],
    ...
    # Compute the Q99%-LBCI for each simulation (it's not the LBCI though! No epistemic uncertainty so no credible interval. It's just the Q99%-LB)
    q99_lbci = {}
    for alpha in DATA_FILES:
        q99_lbci[alpha] = np.quantile(for_lbci.loc[for_lbci['param val'] == alpha, 'time'], 1 - (99 / 100))
    print(q99_lbci)
