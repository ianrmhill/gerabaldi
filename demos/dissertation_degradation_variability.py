...
def run_simulation(save_files: dict = None):
    ...
    ramp_cycle_relax_interval = StrsSpec({'temp': 100 + CELSIUS_TO_KELVIN, 'vdd': 0.8}, 10, 'Ramp Cycle Relax', 'h')
    ...
    # Amplifier design is assumed to have a full DC output bias of Vdd to determine Vds for simplicity
    def hci_vth_shift_zhang(time, vdd, temp, a_0, u, v, alpha, beta, tempref):
        return a_0 * (vdd ** np.maximum(u, 1.0)) * (vdd ** np.maximum(v, 1.0)) * (time ** ((alpha) + ((beta) * (temp - tempref))))
    ...
    def v_th_eqn(init, bti, hci, cond):
        return init + (1.0 * bti) + (1.0 * hci) + cond
            ...
            'v_th': DegPrmMdl(
                deg_mech_mdls={
                    'bti': DegMechMdl(
                        ...
                        e_aa=LatentVar(
                            Normal(-0.05, 0.0002), chp_vrtn_mdl=Normal(1, 0.03), lot_vrtn_mdl=Normal(1, 0.01)
                        ),
                        alpha=LatentVar(Normal(9.5, 0.02), chp_vrtn_mdl=Normal(1, 0.04)),
                        n=LatentVar(Normal(0.4, 0.0005)),
                    ),
                    'hci': DegMechMdl(
                        hci_vth_shift_zhang,
                        a_0=LatentVar(Normal(0.0002, 0.00005)),
                        u=LatentVar(Normal(6.5, 0.002), chp_vrtn_mdl=Normal(1, 0.05), lot_vrtn_mdl=Normal(1, 0.03)),
                        v=LatentVar(Normal(3.7, 0.002), chp_vrtn_mdl=Normal(1, 0.1), lot_vrtn_mdl=Normal(1, 0.05)),
                        alpha=LatentVar(Normal(0.17, 0.005)),
                        beta=LatentVar(Normal(-0.008, 0.0001), chp_vrtn_mdl=Normal(1, 0.15)),
                        tempref=LatentVar(deter_val=300),
                    )
                },
                init_val_mdl=InitValMdl(
                    init_val=LatentVar(
                        Normal(0.42, 0.001),
                        chp_vrtn_mdl=Normal(0, 0.0004),
                        lot_vrtn_mdl=Normal(0, 0.0005),
                        vrtn_type='offset',
                    )
                ),
    ...
    dev_mdl.prm_mdl('v_th').init_mdl.latent_var('init_val').lot_vrtn_mdl = Normal(0, 0.001)
    dev_mdl.prm_mdl('v_th').mech_mdl('bti').latent_var('alpha').chp_vrtn_mdl = Normal(1, 0.08)
    dev_mdl.prm_mdl('v_th').mech_mdl('hci').latent_var('beta').chp_vrtn_mdl = Normal(1, 0.3)
...
def visualize(rprts):
    ...
    maxs, mins, stds, avgs, times, final = {}, {}, {}, {}, {}, {}
    for sim in measured:
    ...
    for sim in measured:
        stds[sim] = []
        for time in times[sim]:
            stds[sim].append(np.std(measured[sim].loc[measured[sim]['time'] == time]['measured']))
        stds[sim] = pd.Series(stds[sim])

    for lot in range(NUM_LOTS):
        for chp in range(NUM_CHIPS):
            for dev in range(NUM_DEVICES):
                for sim in measured:
                    meas = measured[sim].loc[('amp_gain', dev, chp, lot)].reset_index()['measured']
    ...
    order_list = ['Current Process: HTOL', 'New Process: HTOL', 'Current Process: LTOL', 'New Process: LTOL',
                  'Current Process: Ramped Cycling', 'New Process: Ramped Cycling']
    for sim in order_list:
        avgs[sim] /= NUM_DEVICES * NUM_CHIPS * NUM_LOTS
        p1.plot(times[sim], avgs[sim], color=colour_map[sim], label=sim)
        p1.fill_between(times[sim], avgs[sim] + stds[sim], avgs[sim] - stds[sim], color=colour_map[sim], alpha=0.2)

    p1.axhline(20, color='maroon')
    p1.axhspan(0, 20, color='maroon', alpha=0.05)
    p1.text(500, 19.4, 'Design Failure Criterion', fontsize='small', ha='center', color='maroon')
    p1.set(
    ...
    p2.text(1.5, 19.4, 'Design Failure Criterion', fontsize='small', ha='center', color='maroon')
