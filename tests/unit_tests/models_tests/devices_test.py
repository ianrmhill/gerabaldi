...
from gerabaldi.models.devices import LatentModel
...
def test_latent_var(sequential_var):
    ...
    assert (ltnt.chp_vrtn_mdl, ltnt.lot_vrtn_mdl, ltnt.name, ltnt.deter_val, ltnt.vrtn_op) == \
    ...
    assert (ltnt.dev_vrtn_mdl, ltnt.chp_vrtn_mdl, ltnt.lot_vrtn_mdl, ltnt.name, ltnt.deter_val, ltnt.vrtn_op) == \
...
def test_latent_model(sequential_var):
    ...
    mdl = LatentModel(sample_eqn, 'TestModel', y=LatentVar(deter_val=4))
    assert mdl.name == 'TestModel'
    assert mdl.compute(2, 3) == -1
    assert np.allclose(mdl.gen_latent_vals(2, 1, 2)['y'], np.array([[[4, 4]], [[4, 4]]]))


def test_init_val_model_basic(sequential_var):
    mdl = InitValModel(init_val=LatentVar(deter_val=3))
    assert (mdl._latent_init_val.dev_vrtn_mdl, mdl._latent_init_val.chp_vrtn_mdl) == \
           (None, None)
    assert mdl.name is None
    ...
    mdl = InitValModel(init_val_eqn=weird_one, mdl_name='Very Unusual',
...
def test_conditional_shift_model(sequential_var):
    ...
    def some_shifting(temp, vdd, a, b):
        return (a * temp) + (b * (-vdd))
    mdl = CondShiftModel(some_shifting, 'TestShift',
...
def test_deg_mech_model_basic():
    ...
    basic_model = DegMechModel(basic_eqn, x=LatentVar(deter_val=3), y=LatentVar(deter_val=2))

    assert basic_model.name is None
    assert basic_model.unitary == 0
    assert basic_model.compute(-1, 1, z=4) == -4
    basic_model = DegMechModel(basic_eqn, unitary_val=1, mdl_name='hello')
    ...
    assert round(basic_model.calc_equiv_strs_time(8, {'y': 4, 'unused': 3}, 0, {'z': 0.5}), 4) == 4
...
def test_degraded_param(sequential_var):
    ...
    mdl = DegradedParamModel(DegMechModel(degradation, mdl_name='sample_mech',
                             ...
                             InitValModel(init_val=LatentVar(deter_val=2, dev_vrtn_mdl=sequential_var(), vrtn_type='offset')),
                             CondShiftModel(shift,
    ...
    prms = mdl.calc_cond_shifted_vals(3, {'temp': 125, 'vdd': 0.6}, np.array([[[28.2031, 50.1494, 98.8683]]]), latents)
    ...
    equiv_times = mdl.calc_equiv_strs_times({'sample_mech': [[[26.2031, 47.1494, 94.8683]]]}, conditions, init_mech_vals, latents)
