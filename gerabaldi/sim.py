    ...
    for prm in measured_devs:
        ...
        if prm_type == CircPrmMdl:
            for required_prm in dev_mdl.prm_mdl(prm).get_required_prms(dev_mdl.prm_mdl_list):
                ...
                if required_prm not in devs or devs[required_prm] < devs[prm]:
...
def _sim_stress_step(
    ...
    deg_prm_list = [prm for prm in dev_mdl.prm_mdl_list if type(dev_mdl.prm_mdl(prm)) is DegPrmMdl]
    ...
    for prm in deg_prm_list:
        ...
        equiv_times = dev_mdl.prm_mdl(prm).calc_equiv_strs_times(
            ...
            sim_state.curr_deg_mech_vals[prm],
            strs_conds[prm],
            sim_state.init_deg_mech_vals[prm],
            sim_state.latent_var_vals[prm],
        )
    ...
    merged = {
        ...
        'start time': sim_state.elapsed - step.duration,
        'end time': sim_state.elapsed,
    }
...
def simulate(test_def: TestSpec, dev_mdl: DeviceMdl, test_env: PhysTestEnv, init_state: SimState = None) -> SimReport:
    """
    ...
    sim_state = gen_init_state(dev_mdl, quantity_info=test_report) if not init_state else deepcopy(init_state)
